...
function captureStatusReq(data) {
    ...
    return when.promise(function(resolve, reject){
        ...
        pneReq({
            ...
            if (err) {
                logger.error('CaptureStatus request error', {data: data, err: (err && err.stack || err)});
                ...
                if (resData.status === 'declined' || resData.status === 'error' || resData.status === 'filtered') {
                    logger.error('CaptureStatus rejected', {data: data, errMsg: resData['error-message'], errCode: resData['error-code'], resData: resData});
                    ...
                    if (resData.status === 'approved') {
                        logger.info('CaptureStatus approved', {data: data, resData: resData});
                        outData.data.approved = true;
                    } else {
                        logger.debug('CaptureStatus processing', {data: data, resData: resData});
                ...
                logger.error('CaptureStatus rejected with unknown error', {data: data, resData: resData});
