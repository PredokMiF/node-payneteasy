...
function executor (fullKey, tpl, cfg, obj, cb) {
    ...
    // Создаем занчения для объектов, е кого указан def
    _.forEach(tpl.tplKeys, function (key) {
        if (!obj.hasOwnProperty(key) && tpl.tpl[key].hasOwnProperty('def')) {
            obj[key] = tpl.tpl[key].def;
        }
    });
