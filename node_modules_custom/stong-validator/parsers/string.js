...
function fromArr (fullKey, tpl, arrItems, cfg) {
    ...
    if (item === 's' || item === 'str' || item === 'string') {
        obj.t = 's';
    } else {
    ...
    for (i = 1, l = items.length; i < l; i += 1) {
        ...
        switch (item) {
            ...
            case 'min':
                ...
                if (!isFinite(obj.min) || obj.min < 0 || obj.min % 1) {
                    return 'Ошибка шаблона: после min должно идти неотрицательное целое число ' + (fullKey ? (' (поле ' + fullKey + ')') : '') + ' ' + (new Error()).stack;
                }
                break;
            case 'max':
                obj.max = Number(items[++i]);
                if (!isFinite(obj.max) || obj.max < 0 || obj.max % 1) {
                    return 'Ошибка шаблона: после max должно идти целое неотрицательное число число ' + (fullKey ? (' (поле ' + fullKey + ')') : '') + ' ' + (new Error()).stack;
                }
                break;
            default:
